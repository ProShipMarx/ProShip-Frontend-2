...
export default function PostLoadPage() {
  const [form, setForm] = useState({ origin:'', destination:'', equipment:'', weight:'', date:'', rate:'', contact:'' })
  const [msg, setMsg] = useState('')

  const onSubmit = async (e) => {
    e.preventDefault()
    try {
      const res = await postLoad(form)
      setMsg('Load posted (or simulated).')
    } catch (e) {
  ...
  return (
    <div className="max-w-xl mx-auto p-6 bg-white rounded-xl shadow mt-10">
      <h2 className="text-xl font-semibold mb-4">Post a Load</h2>
      <form className="grid gap-3" onSubmit={onSubmit}>
        {['origin','destination','equipment','weight','date','rate','contact'].map(k => (
